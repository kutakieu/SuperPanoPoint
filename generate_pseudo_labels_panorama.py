...
def get_args():
    ...
    parser.add_argument('--dir_in', help="directory of input panorama images", type=str, default="data/homographic/panorama/imgs")
    parser.add_argument('--dir_out', help="directory to save points", type=str, default="data/homographic/panorama/points")
...
def main():
    ...
    for img_path in tqdm(list(dir_in.glob('*'))):
        if (dir_out / f'{img_path.stem}.json').exists():
            continue
        try:
            pano_img = np.array(Image.open(img_path))
            h, w = pano_img.shape[:2]
            pano_transform_adapter = PanoramaTransformAdapter(w, h, detector, num_homographies=100)
            points_json = pano_transform_adapter.generate_pseudo_labels(pano_img)

            vis_img = visualize_points(pano_img, points_json['points'])
