...
def equirect_uvgrid(h: int, w: int):
...
def uv2coor(uv: np.ndarray, h: int, w: int):
    '''
...
def coor2uv(coorxy: np.ndarray, h: int, w: int):
...
def cube_h2dict(cube_h: np.ndarray):
...
def rotation_matrix(rad: float, ax: List[int]):
    ax = np.array(ax)
